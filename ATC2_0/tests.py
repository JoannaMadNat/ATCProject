...
class PlaneTestCases(TestCase):
    ...
    def test_plane_create_validation(self):
        ...
        response = c.post(PLANE_END + '/new',
                          {'identifier': 'P1', 'size': 'something', 'currentPassengerCount': 3000,
                           'maxPassengerCount': 250,
                           'airline': airline.id})
        # should fail
    ...
    def test_user_auth(self):
        c = Client()
        temp = "temp"
        user = User.objects.create_user(username=temp, email=temp, password=temp)
        p = Permission.objects.get(codename="view_plane")
        user.user_permissions.add(p)
        self.assertEqual(user.has_perm("ATC2_0.view_plane"), True)
        self.assertEqual(user.has_perm("ATC2_0.add_airline"), False)

        logged_in = c.login(username=temp, password=temp)
        self.assertEqual(logged_in, True)

        # go in as non-admin user
        response = c.get(PLANE_END)
